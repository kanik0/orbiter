...
| P2-B3 | **RELEASE-BLOCKER** | 1.2 | **M11 post-process pipeline rompe il rendering**: con flag di default (`OGL_NO_POSTFX` unset) tutti gli scenari renderizzano TUTTO NERO (solo HUD visibile) sia interactive sia `--capture-frame`. Root cause: `OGLPostProcess::BeginScene()` bind `m_sceneFBO` ma `EndScene()` non re-composita su backbuffer. Confermato: `OGL_NO_POSTFX=1 ./Orbiter` → Earth visibile. **Il "14/14 PASS" Phase 1 era falso positivo** (size > 50 KB gating inadeguato — HUD text compresso ~183 KB). Baselines rendering_parity 0-byte di M30 sospetto siano stati lasciati placeholder PROPRIO perché prima cattura era nera con postfx on. | `OGLPostProcess::BeginScene/EndScene` pipeline (M11) | root cause identified — **escape hatch OGL_NO_POSTFX=1 disponibile** |
| P2-B4 | high | 1.2 | Con `OGL_NO_POSTFX=1` la Terra appare ma **colori totalmente sbagliati**: continenti VERDI (dovrebbero essere terrain brown/beige + oceani blu). Sembra che canale R sia perso o shader planet/texplanet non campioni correttamente il color ramp. | `shaders/texplanet.frag` o `OGLvPlanet::Render` color sampling | investigating |
| P2-B5 | high | 1.2 | Con `OGL_NO_POSTFX=1` **nessuna stella visibile** in Today scenario (sfondo solid black). Celestial sphere M13 dichiarata ✅ in roadmap ma a runtime il pass starfield non renderizza. | `OGLCelSphere::Render` → `shaders/star.vert/frag` | investigating |
| P2-B6 | medium | 1.2 | Con `OGL_NO_POSTFX=1` **nessun halo/atmosfera** attorno alla Terra. M4 Rayleigh/Mie scattering dichiarato ✅ ma non visibile nello scenario Today. Possibile collegato a P2-B3 (atmospheric glow parte del post-process) o regression separata in `scatter.frag`. | `shaders/scatter.frag` / `OGLvPlanet::Render` atmosphere pass | investigating (potenzialmente risolto con fix P2-B3) |
